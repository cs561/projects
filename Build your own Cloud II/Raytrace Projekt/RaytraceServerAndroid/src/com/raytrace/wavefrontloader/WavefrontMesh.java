...
import com.raytrace.server.MainActivity;

public class WavefrontMesh {

	private ArrayList<Triangle> triangles;
	
	public WavefrontMesh(String meshFile) {
		try {
	...
	private ArrayList<Triangle> parseObjFile(String meshFile) throws Exception {
		...
		InputStream meshStream = MainActivity.getAssetManager().open(meshFile);
	...
	private ArrayList<Texture> parseMtlFile(String mtlFile) throws Exception {
		ArrayList<Texture> textures = new ArrayList<Texture>();
		
		InputStream mtlStream = MainActivity.getAssetManager().open(mtlFile);
		...
		while ((line = mtlReader.readLine()) != null) {
			...
			if (parts[0].equalsIgnoreCase("newmtl")) {
				...
				if (parts.length > 1) {
					textures.add(new Texture(currentName, BitmapFactory.decodeStream(MainActivity.getAssetManager().open(parts[1]))));
				}
			}
		}
